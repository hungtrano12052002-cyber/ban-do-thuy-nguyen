...
/* V7.5.2 Command Center dashboard */
function v752Dashboard(){
 const k=document.getElementById('ccKpis'), a=document.getElementById('ccAlerts'), st=document.getElementById('ccDataState');
 if(!k)return;
 const visible=filtered();
 const residents=visible.reduce((n,x)=>n+(Array.isArray(x.residents)?x.residents.length:0),0);
 const activeResidents=visible.reduce((n,x)=>n+(Array.isArray(x.residents)?x.residents.filter(r=>!/rời|tam vang|tạm vắng/i.test(String(r.status||''))).length:0),0);
 const temp=visible.reduce((n,x)=>n+(Array.isArray(x.residents)?x.residents.filter(r=>/tạm vắng|tam vang/i.test(String(r.status||''))).length:0),0);
 const left=Math.max(0,residents-activeResidents-temp);
 k.innerHTML=`<div class="cc-kpi"><small>🏠 Tổng số cơ sở</small><b>${visible.length}</b></div><div class="cc-kpi"><small>👥 Tổng người cư trú</small><b>${residents}</b></div><div class="cc-kpi"><small>● Đang cư trú</small><b>${activeResidents}</b></div><div class="cc-kpi"><small>◉ Tạm vắng</small><b>${temp}</b></div><div class="cc-kpi"><small>○ Đã rời đi</small><b>${left}</b></div>`;
 const noGeo=visible.filter(x=>!validGeo(x)).length, noSheet=visible.filter(x=>!lodgerSheetUrl(x)).length, noOfficer=visible.filter(x=>!String(x.officer||'').trim()).length;
 if(a)a.innerHTML=`<div class="cc-alert"><b>📍 Cơ sở chưa có tọa độ</b><em>${noGeo} cơ sở</em> cần cập nhật vị trí</div><div class="cc-alert"><b>📊 Chưa gắn Google Sheets</b><em>${noSheet} cơ sở</em> chưa liên kết danh sách</div><div class="cc-alert"><b>👮 Chưa phân công cán bộ</b><em>${noOfficer} cơ sở</em> cần rà soát</div>`;
 if(st)st.textContent=currentUser?.cloud?'☁ Đang đồng bộ Cloud':'◉ Dữ liệu cục bộ';
}
const _v752RenderAll=renderAll; renderAll=function(){_v752RenderAll();v752Dashboard()};

/* ===== V7.5.3 SMART COMMAND CENTER ===== */
function v753ResidentStats(arr=filtered()){
 const all=arr.flatMap(x=>Array.isArray(x.residents)?x.residents:[]);
 const temp=all.filter(r=>/tạm vắng|tam vang/i.test(String(r.status||''))).length;
 const left=all.filter(r=>/rời|roi|đã đi|da di/i.test(String(r.status||''))).length;
 return {total:all.length,temp,left,active:Math.max(0,all.length-temp-left)};
}
function v753SetNav(name){document.querySelectorAll('.cc-nav button').forEach(b=>b.classList.toggle('active',b.textContent.trim().toLowerCase().includes(name)))}
function v753ShowDashboard(){
 const d=$('smartDashboard');if(!d)return;const arr=filtered(),rs=v753ResidentStats(arr),noGeo=arr.filter(x=>!validGeo(x)).length,noSheet=arr.filter(x=>!lodgerSheetUrl(x)).length,noOfficer=arr.filter(x=>!String(x.officer||'').trim()).length;
 const recent=arr.slice(0,5);
 d.innerHTML=`<div class="sd-head"><div><small>TRUNG TÂM ĐIỀU HÀNH</small><h2>Tổng quan quản lý cơ sở lưu trú</h2><p>Dữ liệu được tổng hợp theo bộ lọc và nguồn dữ liệu hiện tại.</p></div><div class="sd-head-actions"><button onclick="v753ShowMap()">🗺 Mở bản đồ</button><button class="primary admin-only" onclick="openAddSafely(event)">＋ Thêm cơ sở</button></div></div>
 <div class="sd-kpis"><article><span>🏠</span><div><small>Tổng số cơ sở</small><b>${arr.length}</b><em>${arr.filter(x=>isActive(x.status)).length} đang hoạt động</em></div></article><article><span>👥</span><div><small>Tổng người cư trú</small><b>${rs.total}</b><em>Dữ liệu đã nhập trực tiếp</em></div></article><article><span>●</span><div><small>Đang cư trú</small><b>${rs.active}</b><em>${rs.total?Math.round(rs.active/rs.total*100):0}% tổng cư trú</em></div></article><article><span>◉</span><div><small>Tạm vắng</small><b>${rs.temp}</b><em>Cần theo dõi</em></div></article><article><span>○</span><div><small>Đã rời đi</small><b>${rs.left}</b><em>Đã cập nhật trạng thái</em></div></article></div>
 <div class="sd-grid"><section class="sd-card sd-map"><div class="sd-card-title"><div><b>🗺 Bản đồ địa bàn</b><small>${arr.filter(validGeo).length} cơ sở có tọa độ</small></div><button onclick="v753ShowMap()">Xem toàn màn hình →</button></div><div id="sdMapPreview" class="sd-map-preview"></div></section>
 <section class="sd-card"><div class="sd-card-title"><div><b>⚠ Cảnh báo & nhắc nhở</b><small>Các dữ liệu cần hoàn thiện</small></div></div><div class="sd-warnings"><button onclick="showUnlocated()"><span>📍</span><div><b>Chưa có tọa độ</b><small>Cập nhật vị trí bản đồ</small></div><strong>${noGeo}</strong></button><button onclick="v753ShowList()"><span>📊</span><div><b>Chưa có Google Sheets</b><small>Bổ sung danh sách cư trú</small></div><strong>${noSheet}</strong></button><button onclick="v753ShowList()"><span>👮</span><div><b>Chưa phân công cán bộ</b><small>Kiểm tra và xác minh</small></div><strong>${noOfficer}</strong></button></div></section>
 <section class="sd-card"><div class="sd-card-title"><div><b>🏢 Cơ sở gần đây</b><small>Truy cập nhanh thông tin cơ sở</small></div><button onclick="v753ShowList()">Xem tất cả</button></div><div class="sd-recent">${recent.map(x=>`<button onclick="v753OpenPlace('${escJs(x.id)}')"><span class="sd-dot ${isActive(x.status)?'on':''}"></span><div><b>${esc(x.name)}</b><small>${esc(x.wardBlock||'Chưa xác định TDP')} · ${esc(x.officer||'Chưa phân công')}</small></div><em>›</em></button>`).join('')||'<p>Chưa có dữ liệu.</p>'}</div></section>
 <section class="sd-card"><div class="sd-card-title"><div><b>📈 Phân bố cơ sở</b><small>Theo loại hình</small></div><button onclick="openOverview()">Báo cáo</button></div><div class="sd-chart"><canvas id="sdTypeChart"></canvas></div></section></div>`;
 d.classList.remove('hidden');document.body.classList.add('dashboard-mode');v753SetNav('tổng quan');
 requestAnimationFrame(()=>{const rows=countBy('category',arr).slice(0,8);makePie('sdTypeChart',rows);v753PreviewMap(arr)});
}
function v753PreviewMap(arr){const box=$('sdMapPreview');if(!box||!window.L)return;try{if(window.__sdMap)window.__sdMap.remove();const m=L.map(box,{zoomControl:false,attributionControl:false,dragging:false,scrollWheelZoom:false,doubleClickZoom:false}).setView(defaultCenter,12);window.__sdMap=m;L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18}).addTo(m);arr.filter(validGeo).slice(0,120).forEach(x=>L.circleMarker([x.lat,x.lng],{radius:4,weight:1,fillOpacity:.9}).addTo(m));const pts=arr.filter(validGeo).map(x=>[x.lat,x.lng]);if(pts.length)m.fitBounds(pts,{padding:[18,18],maxZoom:13});setTimeout(()=>m.invalidateSize(),50)}catch(e){}
}
function v753HideDashboard(){document.body.classList.remove('dashboard-mode');$('smartDashboard')?.classList.add('hidden')}
function v753ShowMap(){v753HideDashboard();showMobileTab('map');v753SetNav('bản đồ');setTimeout(v75RefreshMap,80)}
function v753ShowList(){v753HideDashboard();showMobileTab('list');v753SetNav('danh sách')}
function v753OpenPlace(id){v753HideDashboard();v753ShowList();v75OpenFromList(id)}
function v753OpenResidents(){
 const rows=filtered().flatMap(p=>(Array.isArray(p.residents)?p.residents:[]).map(r=>({...r,placeName:p.name,wardBlock:p.wardBlock,placeId:p.id})));
 $('modalCard').innerHTML=`<div class="modal-head"><h2>👥 Danh sách người cư trú (${rows.length})</h2><button onclick="closeModal()">×</button></div><div class="resident-page-tools"><input id="v753rq" placeholder="🔎 Tìm họ tên, CCCD, SĐT, cơ sở..." oninput="v753FilterResidents()"><button onclick="v73ExportResidentsCsv()">⬇ Xuất CSV</button></div><div id="v753ResidentTable" class="resident-page-table"></div>`;$('modal').classList.remove('hidden');window.__v753Residents=rows;v753RenderResidents(rows);v753SetNav('người cư trú');
}
function v753FilterResidents(){const q=normSearch($('v753rq')?.value||'');v753RenderResidents((window.__v753Residents||[]).filter(r=>!q||normSearch(Object.values(r).join(' ')).includes(q)))}
function v753RenderResidents(rows){const b=$('v753ResidentTable');if(!b)return;b.innerHTML=rows.length?`<table><thead><tr><th>Họ tên</th><th>Năm sinh</th><th>CCCD</th><th>SĐT</th><th>Cơ sở</th><th>TDP</th><th>Trạng thái</th></tr></thead><tbody>${rows.map(r=>`<tr onclick="closeModal();v753OpenPlace('${escJs(r.placeId)}')"><td><b>${esc(r.name||'—')}</b></td><td>${esc(r.birthYear||r.year||'—')}</td><td>${esc(r.cccd||'—')}</td><td>${esc(r.phone||'—')}</td><td>${esc(r.placeName||'—')}</td><td>${esc(r.wardBlock||'—')}</td><td><span class="resident-status">${esc(r.status||'Đang ở')}</span></td></tr>`).join('')}</tbody></table>`:'<div class="v7-empty">Chưa có dữ liệu người cư trú dạng bảng.</div>'}
const _v753EnterApp=enterApp;enterApp=function(){_v753EnterApp();setTimeout(v753ShowDashboard,120)};
const _v753RenderAll=renderAll;renderAll=function(){_v753RenderAll();if(document.body.classList.contains('dashboard-mode'))setTimeout(v753ShowDashboard,0)};

/* ===== V7.5.4 FACILITY PHOTO + COMPLETE PROFILE ===== */
function v754ImageList(x){return (Array.isArray(x?.images)?x.images:[]).map(v75ImageSrc).filter(Boolean).slice(0,12)}
function v754Cover(x){return v754ImageList(x)[0]||''}
function v754Completion(x){
 const checks=[x.name,x.category,x.wardBlock,x.ownerName,x.ownerPhone,x.managerName,x.managerPhone,x.officer,x.scale,x.legal,x.status,x.mapsUrl,validGeo(x)?'geo':'',v754ImageList(x).length?'img':'',lodgerSheetUrl(x)||v72ResidentArray(x).length||String(x.residentsText||'').trim()?'resident':''];
 const done=checks.filter(Boolean).length,pct=Math.round(done/checks.length*100);return {done,total:checks.length,pct};
}
function v754PhotoHtml(x,cls=''){const src=v754Cover(x);return src?`<div class="v754-cover ${cls}"><img src="${escAttr(src)}" alt="Ảnh ${escAttr(x.name||'cơ sở')}" loading="lazy" decoding="async" onerror="this.parentElement.classList.add('broken');this.remove()"></div>`:`<div class="v754-cover v754-no-photo ${cls}"><span>🏠</span><small>Chưa có ảnh</small></div>`}
renderList=function(){
 const base=filtered(),q=normSearch(listQuery||'');
 const arr=q?base.filter(x=>normSearch([x.name,x.category,x.wardBlock,x.ownerName,x.managerName,x.officer,x.ownerPhone,x.managerPhone,x.lodgerListUrl,v72AllResidentSearch(x)].join(' ')).includes(q)):base;
 const list=$('list');if(!list)return;
 const search=`<div class="list-search"><span>🔎</span><input value="${escAttr(listQuery||'')}" placeholder="Tìm cơ sở hoặc người cư trú…" oninput="setListQuery(this.value)"><button type="button" onclick="setListQuery('')">×</button></div><div class="list-count">${arr.length} / ${base.length} cơ sở</div>`;
 const rows=arr.length?arr.map(x=>{const st=v73ResidentStats(x),sheet=lodgerSheetUrl(x),cp=v754Completion(x);return `<article class="item v7-list-item v71-list-item v754-list-card ${selectedId===x.id?'selected':''}" data-id="${escAttr(x.id)}" tabindex="0" role="button">${v754PhotoHtml(x,'v754-list-photo')}<div class="v754-list-body"><div class="item-head"><b>${esc(x.name||'Chưa đặt tên')}</b><span class="dot ${isActive(x.status)?'on':'off'}"></span></div><small>${esc(x.category||'Cơ sở')} · ${esc(x.wardBlock||'Chưa có TDP')}</small><small>👮 ${esc(x.officer||'Chưa phân công')}</small><div class="v754-meta"><span>👥 ${st.total||v71ResidentCount(x)||0}</span>${sheet?'<span>📊 Sheet</span>':''}<span class="v754-complete ${cp.pct<70?'warn':''}">✓ ${cp.pct}%</span></div><div class="item-foot"><span class="badge ${isActive(x.status)?'':'off'}">${esc(x.status||'Chưa rõ')}</span><span class="v7-open">Xem thông tin ›</span></div></div></article>`}).join(''):`<div class="empty">Không có kết quả phù hợp.</div>`;
 list.innerHTML=search+rows;
};
renderMarkers=function(){
 if(!map)return;if(markerCluster)markerCluster.clearLayers();markers=[];
 filtered().forEach(x=>{if(!validGeo(x))return;const m=L.marker([x.lat,x.lng],{icon:markerIcon(isActive(x.status),x.name,String(selectedId)===String(x.id)),riseOnHover:true,title:x.name});
  const cover=v754Cover(x),cp=v754Completion(x);m.bindPopup(`<div class="v754-map-card">${cover?`<img src="${escAttr(cover)}" alt="Ảnh cơ sở" onerror="this.remove()">`:'<div class="v754-map-noimg">🏠</div>'}<div><b>${esc(x.name||'Cơ sở')}</b><small>${esc(x.category||'Cơ sở')} · ${esc(x.wardBlock||'Chưa có TDP')}</small><span>Hoàn thiện ${cp.pct}%</span><button type="button" onclick="showDetail(places.find(p=>String(p.id)==='${escJs(x.id)}'))">Xem chi tiết</button></div></div>`,{maxWidth:310,className:'v754-popup'});
  m.bindTooltip(`<div class="map-tip"><b>${esc(x.name)}</b><span>${esc(x.category||'Cơ sở')}</span><small>${esc(x.wardBlock||'Chưa có TDP')}</small></div>`,{direction:'top',offset:[0,-34],opacity:.98});
  m.on('click',()=>{selectedId=x.id;renderList();setTimeout(()=>m.openPopup(),20)});if(markerCluster)markerCluster.addLayer(m);else m.addTo(map);markers.push(m)
 });updateMapNotice();
}
function v754DetailTab(tab){document.querySelectorAll('.v754-tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));document.querySelectorAll('.v754-pane').forEach(p=>p.classList.toggle('active',p.dataset.pane===tab))}
showDetail=function(x){
 if(!x)return;selectedId=x.id;renderList();const d=$('detail');if(!d)return;d.classList.remove('hidden');const imgs=v754ImageList(x),cp=v754Completion(x),res=v72ResidentArray(x);
 const gallery=imgs.length?`<div class="v754-gallery">${imgs.map((src,i)=>`<button onclick="v754Lightbox('${escJs(src)}')"><img src="${escAttr(src)}" alt="Ảnh cơ sở ${i+1}" loading="lazy" onerror="this.closest('button').remove()"></button>`).join('')}</div>`:`<div class="v754-empty-photo"><span>📷</span><b>Chưa có ảnh cơ sở</b><small>ADMIN có thể bổ sung ảnh trong Chỉnh sửa.</small></div>`;
 d.innerHTML=`<button class="close" onclick="dclose()">×</button><div class="detail-title v754-title">${v754PhotoHtml(x,'v754-hero')}<div><h2>${esc(x.name||'Cơ sở')}</h2><p>${esc(x.category||'Cơ sở')} · ${esc(x.wardBlock||'Chưa có TDP')}</p><span class="badge ${isActive(x.status)?'':'off'}">${esc(x.status||'Chưa rõ')}</span></div></div><div class="v754-progress"><div><b>Hồ sơ cơ sở</b><span>${cp.pct}% hoàn thiện</span></div><i><em style="width:${cp.pct}%"></em></i></div><div class="v754-tabs"><button class="v754-tab active" data-tab="info" onclick="v754DetailTab('info')">Thông tin</button><button class="v754-tab" data-tab="res" onclick="v754DetailTab('res')">Người cư trú <span>${res.length}</span></button><button class="v754-tab" data-tab="photo" onclick="v754DetailTab('photo')">Hình ảnh <span>${imgs.length}</span></button></div><section class="v754-pane active" data-pane="info">${row('Chủ cơ sở',x.ownerName)}${row('Số điện thoại',phoneLink(x.ownerPhone))}${row('Người quản lý',x.managerName)}${row('SĐT quản lý',phoneLink(x.managerPhone))}${row('Địa chỉ quản lý',x.managerAddress)}${row('Cán bộ phụ trách',x.officer)}${row('Quy mô',x.scale)}${row('Pháp lý',x.legal)}${row('Tọa độ',validGeo(x)?`${x.lat}, ${x.lng}`:'Chưa có')}</section><section class="v754-pane" data-pane="res">${v72StructuredResidentBlock(x)||'<div class="v7-empty">Chưa nhập danh sách người cư trú.</div>'}${lodgerSheetUrl(x)?`<a class="v754-sheet" href="${escAttr(lodgerSheetUrl(x))}" target="_blank" rel="noopener">📊 Mở Google Trang tính</a>`:''}</section><section class="v754-pane" data-pane="photo">${gallery}</section><div class="actions mobile-actions v754-actions"><a class="route-a" href="${escAttr(directionUrl(x))}" target="_blank" rel="noopener">🧭 Chỉ đường</a>${x.mapsUrl?`<a href="${escAttr(x.mapsUrl)}" target="_blank" rel="noopener">📍 Google Maps</a>`:''}<button class="secondary" onclick="sharePlace('${escJs(x.id)}')">↗ Chia sẻ</button>${String(currentUser?.role||'').toLowerCase()==='admin'?`<button class="direct-edit" onclick="openEditor('${escJs(x.id)}')">✏️ Chỉnh sửa & hoàn thiện</button>`:''}</div>`;
}
function v754Lightbox(src){const m=$('modal'),c=$('modalCard');c.innerHTML=`<div class="v754-lightbox"><button onclick="closeModal()">×</button><img src="${escAttr(src)}" alt="Ảnh cơ sở"></div>`;m.classList.remove('hidden')}
function v754AddImageUrl(){const input=$('v754ImageUrl'),url=String(input?.value||'').trim();if(!safeHttpUrl(url))return toast('Link ảnh phải bắt đầu bằng http:// hoặc https://');const box=document.createElement('div');box.className='img-box';box.innerHTML=`<img src="${escAttr(url)}"><button type="button" onclick="this.parentElement.remove()">×</button>`;$('imagePreview')?.appendChild(box);input.value='';}
async function v754CompressImage(file){if(!file.type.startsWith('image/'))return file;return new Promise(resolve=>{const img=new Image(),u=URL.createObjectURL(file);img.onload=()=>{const max=1600,s=Math.min(1,max/Math.max(img.width,img.height)),c=document.createElement('canvas');c.width=Math.round(img.width*s);c.height=Math.round(img.height*s);c.getContext('2d').drawImage(img,0,0,c.width,c.height);c.toBlob(b=>{URL.revokeObjectURL(u);resolve(b?new File([b],(file.name||'photo').replace(/\.[^.]+$/,'.jpg'),{type:'image/jpeg'}):file)},'image/jpeg',.82)};img.onerror=()=>{URL.revokeObjectURL(u);resolve(file)};img.src=u})}
previewImages=async function(e){const files=[...e.target.files].slice(0,12);for(const raw of files){if(raw.size>12_000_000){toast('Bỏ qua ảnh lớn hơn 12 MB.');continue}const f=await v754CompressImage(raw),data=await fileToDataUrl(f),div=document.createElement('div');div.className='img-box';div._file=f;div.innerHTML=`<img src="${data}"><button type="button" onclick="this.parentElement.remove()">×</button>`;$('imagePreview')?.appendChild(div)}};
const _v754OpenEditor=openEditor;openEditor=function(id=null){_v754OpenEditor(id);setTimeout(()=>{const ed=document.querySelector('.image-editor');if(ed&&!$('v754ImageUrl')){ed.insertAdjacentHTML('afterbegin',`<div class="v754-image-head"><div><b>📷 Hình ảnh cơ sở</b><small>Chụp/tải nhiều ảnh hoặc gắn link ảnh. Ảnh sẽ tự nén để tải nhanh hơn.</small></div></div>`);const input=ed.querySelector('#f_images');if(input){input.setAttribute('accept','image/*');input.setAttribute('capture','environment')}ed.insertAdjacentHTML('beforeend',`<div class="v754-image-url"><input id="v754ImageUrl" type="url" placeholder="Hoặc dán link ảnh https://..."><button type="button" onclick="v754AddImageUrl()">＋ Thêm link ảnh</button></div>`)}},30)};

/* ===== V7.5.5 SUPER ADMIN + TDP ACCESS CONTROL ===== */
const V755_ACCOUNTS='thuynguyen_accounts_v755';
function v755IsSuper(){return String(currentUser?.role||'').toLowerCase()==='admin'}
function v755NormTdp(v){return normSearch(String(v||'').replace(/^tổ dân phố\s*/i,'').replace(/^tdp\s*/i,''))}
function v755AllowedTdp(x){if(v755IsSuper())return true;const scopes=Array.isArray(currentUser?.tdpAccess)?currentUser.tdpAccess:[];if(!scopes.length)return false;const w=v755NormTdp(x?.wardBlock);return scopes.some(s=>v755NormTdp(s)===w)}
function v755CanEdit(x){return v755IsSuper() || (!!currentUser?.canEdit && v755AllowedTdp(x))}
function v755Accounts(){try{return JSON.parse(localStorage.getItem(V755_ACCOUNTS)||'[]')}catch(e){return []}}
function v755SaveAccounts(a){localStorage.setItem(V755_ACCOUNTS,JSON.stringify(a))}
function v755TdpList(){return [...new Set(places.map(x=>String(x.wardBlock||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'vi',{numeric:true}))}
async function v755Hash(pass,salt){const data=new TextEncoder().encode(`${salt}|${pass}`),buf=await crypto.subtle.digest('SHA-256',data);return [...new Uint8Array(buf)].map(b=>b.toString(16).padStart(2,'0')).join('')}
function v755Salt(){return crypto.getRandomValues(new Uint32Array(4)).join('-')}
async function v755LocalLogin(u,p){const a=v755Accounts().find(x=>normSearch(x.username)===normSearch(u)&&x.enabled!==false);if(!a)return null;const h=await v755Hash(p,a.salt);if(h!==a.passwordHash)return null;return {name:a.username,role:'user',cloud:false,localManaged:true,tdpAccess:a.tdpAccess||[],canEdit:!!a.canEdit,id:a.id}}
login=async function(){
 const u=$('user').value.trim(),p=$('pass').value;
 if(u==='admin'&&p===demoAuth.admin.pass){currentUser={name:'admin',role:'admin',cloud:false,tdpAccess:['*'],canEdit:true};sessionStorage.setItem('demo_user','admin');enterApp();return}
 const local=await v755LocalLogin(u,p);if(local){currentUser=local;sessionStorage.setItem('v755_user',JSON.stringify(local));enterApp();return}
 if(cloudEnabled&&u.includes('@')){const {data,error}=await supabaseClient.auth.signInWithPassword({email:u,password:p});if(error)return toast('Đăng nhập thất bại: '+error.message);let prof={role:'user',tdp_access:[],can_edit:false};try{const r=await supabaseClient.from('profiles').select('role,tdp_access,can_edit').eq('id',data.user.id).maybeSingle();if(r.data)prof=r.data}catch(e){}currentUser={name:data.user.email,role:prof.role==='admin'?'admin':'user',cloud:true,id:data.user.id,tdpAccess:prof.tdp_access||[],canEdit:!!prof.can_edit};enterApp();return}
 toast('Sai tài khoản/mật khẩu hoặc tài khoản đã bị khóa.');
}
const _v755Filtered=filtered;filtered=function(){return _v755Filtered().filter(v755AllowedTdp)};
const _v755EnterApp=enterApp;enterApp=function(){_v755EnterApp();const r=$('role');if(r&&!v755IsSuper())r.textContent=`USER · ${currentUser.tdpAccess?.length||0} TDP`;document.body.classList.toggle('v755-scoped',!v755IsSuper());setTimeout(v755ScopeBanner,80)};
function v755ScopeBanner(){let e=document.getElementById('v755Scope');if(!e){e=document.createElement('div');e.id='v755Scope';e.className='v755-scope';document.querySelector('header')?.insertAdjacentElement('afterend',e)}if(v755IsSuper()){e.innerHTML='👑 <b>Tài khoản chủ:</b> Toàn bộ dữ liệu · Toàn bộ TDP';e.classList.add('super')}else{e.innerHTML=`🔐 Phạm vi: <b>${(currentUser.tdpAccess||[]).map(esc).join(', ')||'Chưa được cấp TDP'}</b>${currentUser.canEdit?' · Được chỉnh sửa':' · Chỉ xem'}`;e.classList.remove('super')}}
const _v755OpenEditor=openEditor;openEditor=function(id=null){if(id!==null){const x=places.find(p=>String(p.id)===String(id));if(!v755CanEdit(x))return toast('Tài khoản không có quyền chỉnh sửa cơ sở này.')}else if(!v755IsSuper())return toast('Chỉ tài khoản chủ được thêm cơ sở mới.');return _v755OpenEditor(id)};
const _v755ShowDetail=showDetail;showDetail=function(x){if(!v755AllowedTdp(x))return toast('Cơ sở nằm ngoài phạm vi TDP được phân quyền.');_v755ShowDetail(x);setTimeout(()=>{if(!v755CanEdit(x))document.querySelectorAll('#detail .direct-edit').forEach(b=>b.remove())},0)};
async function v755OpenAccounts(){if(!v755IsSuper())return toast('Chỉ tài khoản chủ ADMIN được quản lý phân quyền.');const a=v755Accounts(),tdps=v755TdpList(),c=$('modalCard');c.innerHTML=`<div class="modal-head"><h2>👥 Tài khoản & phân quyền TDP</h2><button onclick="closeModal()">×</button></div><div class="v755-note"><b>ADMIN</b> là tài khoản chủ, luôn có toàn quyền và xem toàn bộ TDP. Tài khoản phụ chỉ thấy dữ liệu TDP được gán.</div><div class="v755-account-actions"><button class="primary" onclick="v755EditAccount()">＋ Tạo tài khoản</button><button onclick="v755ExportAccounts()">⬇ Xuất cấu hình</button><label class="v755-import">⬆ Nhập cấu hình<input type="file" accept="application/json,.json" onchange="v755ImportAccounts(event)" hidden></label></div><div class="v755-account-list">${a.length?a.map(x=>`<div class="v755-account"><div><b>${esc(x.username)}</b><small>${x.enabled===false?'🔒 Đã khóa':'● Hoạt động'} · ${x.canEdit?'Có quyền sửa':'Chỉ xem'}</small><span>${(x.tdpAccess||[]).map(t=>`<i>${esc(t)}</i>`).join('')||'<i>Chưa cấp TDP</i>'}</span></div><button onclick="v755EditAccount('${escJs(x.id)}')">Sửa</button><button class="danger" onclick="v755DeleteAccount('${escJs(x.id)}')">Xóa</button></div>`).join(''):'<div class="empty">Chưa có tài khoản phụ.</div>'}</div><div class="v755-foot">Có ${tdps.length} TDP được nhận diện từ dữ liệu cơ sở.</div>`;$('modal').classList.remove('hidden')}
function v755EditAccount(id=''){const old=v755Accounts().find(x=>String(x.id)===String(id)),tdps=v755TdpList(),c=$('modalCard');c.innerHTML=`<div class="modal-head"><h2>${old?'Sửa':'Tạo'} tài khoản phụ</h2><button onclick="v755OpenAccounts()">←</button></div><form onsubmit="v755SaveAccount(event,'${escJs(id)}')"><label>Tên đăng nhập<input id="v755_user" required value="${escAttr(old?.username||'')}"></label><label>${old?'Mật khẩu mới (để trống nếu giữ nguyên)':'Mật khẩu'}<input id="v755_pass" type="password" ${old?'':'required'} minlength="6"></label><label class="check"><input id="v755_enabled" type="checkbox" ${old?.enabled===false?'':'checked'}> Tài khoản hoạt động</label><label class="check"><input id="v755_edit" type="checkbox" ${old?.canEdit?'checked':''}> Cho phép chỉnh sửa cơ sở thuộc TDP được giao</label><div class="v755-tdp-head"><b>Phân quyền theo Tổ dân phố</b><button type="button" onclick="document.querySelectorAll('.v755-tdp input').forEach(x=>x.checked=true)">Chọn tất cả</button><button type="button" onclick="document.querySelectorAll('.v755-tdp input').forEach(x=>x.checked=false)">Bỏ chọn</button></div><div class="v755-tdp">${tdps.map(t=>`<label><input type="checkbox" value="${escAttr(t)}" ${(old?.tdpAccess||[]).includes(t)?'checked':''}> ${esc(t)}</label>`).join('')}</div><div class="modal-actions"><button type="button" onclick="v755OpenAccounts()">Hủy</button><button class="primary" type="submit">💾 Lưu phân quyền</button></div></form>`}
async function v755SaveAccount(e,id){e.preventDefault();const a=v755Accounts(),username=$('v755_user').value.trim();if(username.toLowerCase()==='admin')return toast('Tên admin được dành cho tài khoản chủ.');if(a.some(x=>x.id!==id&&normSearch(x.username)===normSearch(username)))return toast('Tên đăng nhập đã tồn tại.');let obj=a.find(x=>String(x.id)===String(id));if(!obj){obj={id:crypto.randomUUID?crypto.randomUUID():String(Date.now()),salt:v755Salt()};a.push(obj)}obj.username=username;obj.enabled=$('v755_enabled').checked;obj.canEdit=$('v755_edit').checked;obj.tdpAccess=[...document.querySelectorAll('.v755-tdp input:checked')].map(x=>x.value);const pass=$('v755_pass').value;if(pass){obj.salt=v755Salt();obj.passwordHash=await v755Hash(pass,obj.salt)}v755SaveAccounts(a);toast('Đã lưu tài khoản và phạm vi TDP.');v755OpenAccounts()}
...
const _v755Restore=restoreSession;restoreSession=async function(){const s=sessionStorage.getItem('v755_user');if(s){try{currentUser=JSON.parse(s);enterApp();return}catch(e){}}return _v755Restore()};

/* ===== V7.5.7 FOCUSED WORKSPACE ===== */
function v757Mode(mode){
 document.body.classList.remove('focus-map','focus-list');
 if(mode==='map')document.body.classList.add('focus-map');
 if(mode==='list')document.body.classList.add('focus-list');
}
const _v757Dash=v753ShowDashboard;v753ShowDashboard=function(){v757Mode('dashboard');return _v757Dash()};
